...
self.addEventListener('fetch', function (event) {
  ...
  //            above (CACHE_NAME)
  event.respondWith(
    caches.open(CACHE_NAME).then(function (cache) {     // open cache with name
      return cache.match(event.request).then(function (response) {    
        if (response) {return response}       // if in cache, use cache

        return fetch(event.request).then(function (network) {     // fetch otherwise
